...
export const createRectangle = ({ x, y, color, strokeWidth }) => {
  setState((state) => {
    state.shapes[nanoid()] = {
      type: SHAPE_TYPES.RECT,
      width: DEFAULTS_SHAPE_VALUE.RECT.WIDTH,
      height: DEFAULTS_SHAPE_VALUE.RECT.HEIGHT,
      fill: DEFAULTS_SHAPE_VALUE.RECT.FILL,
      stroke: color,
      rotation: DEFAULTS_SHAPE_VALUE.RECT.ROTATION,
...
export const createCircle = ({ x, y, color, strokeWidth }) => {
  setState((state) => {
    state.shapes[nanoid()] = {
      type: SHAPE_TYPES.CIRCLE,
      radius: DEFAULTS_SHAPE_VALUE.CIRCLE.RADIUS,
      fill: DEFAULTS_SHAPE_VALUE.CIRCLE.FILL,
      stroke: color,
      strokeWidth: strokeWidth,
      x,
      y,
  ...
  setState((state) => {
    ...
    if (shape) {
      ...
      shape.width = clamp(
        ...
        LIMITS_SHAPE_VALUE.RECT.MIN,
        // should not be more than the maximum width
        LIMITS_SHAPE_VALUE.RECT.MAX
      );
      shape.height = clamp(
        node.height() * scaleY,
        LIMITS_SHAPE_VALUE.RECT.MIN,
...
export const transformCircleShape = (node, id, event) => {
  ...
  setState((state) => {
    ...
    if (shape) {
      ...
      shape.radius = clamp(
        (node.width() * scaleX) / 2,
        LIMITS_SHAPE_VALUE.CIRCLE.MIN,
        LIMITS_SHAPE_VALUE.CIRCLE.MAX
      );
    }
  });
};
